...
import {
  ...
  useGLTF,
  useScroll,
  Image, // Behalten, falls Sie Images später wieder aktivieren möchten
  Scroll,
  Preload,
  ScrollControls,
    ...
    <Canvas camera={{ position: [0, 0, 20], fov: 15 }} gl={{ alpha: true }}>
      <ScrollControls damping={0.2} pages={3} distance={0.4}>
        ...
        <Wrapper modeProps={modeProps}>
          <Scroll>
            <Typography />
            {/* <Images /> ANPASSUNG: Entfernt, um den Fokus auf den Text "NOHA" zu legen */}
          </Scroll>
          <Scroll html />
          <Preload />
        </Wrapper>
...
const ModeWrapper = memo(function ModeWrapper({
  ...
  useFrame((state, delta) => {
    ...
    // Hintergrundfarbe (kann angepasst oder entfernt werden, wenn transparent gewünscht)
    gl.setClearColor(0x000000, 0); // Transparenten Hintergrund erzwingen, anstatt 0x5227ff
  });
...
function NavItems({ items }) {
  ...
  // --- KORREKTUR FÜR SSR/INITIALISIERUNG ---
  ...
  }, []); // DEVICE als Dependency hinzugefügt, falls es sich ändert (obwohl es konstant ist)

  const [device, setDevice] = useState("desktop"); // Standardwert setzen

  useEffect(() => {
    setDevice(getDevice()); // Echten Wert nach dem Mounten setzen
...
function Typography() {
  const DEVICE = {
    mobile: { fontSize: 0.6 },
    tablet: { fontSize: 0.8 },
    desktop: { fontSize: 1.2 },
  };

  // --- KORREKTUR FÜR 500-FEHLER START ---

  // 1. Erstellen Sie die Funktion zum Abrufen der Gerätegröße.
  const getDevice = useCallback(() => {
    // Überprüfen, ob window definiert ist, bevor darauf zugegriffen wird.
    if (typeof window !== "undefined") {
      const w = window.innerWidth;
      return w <= 639 ? "mobile" : w <= 1023 ? "tablet" : "desktop";
    }
    return "desktop"; // Standard-Fallback
  }, []);

  // 2. Setzen Sie den initialen State auf einen statischen Standardwert (z.B. "desktop").
  const [device, setDevice] = useState("desktop");

  // 3. Verwenden Sie useEffect, um den State clientseitig zu aktualisieren, sobald window verfügbar ist.
  useEffect(() => {
    setDevice(getDevice()); // Setzt den korrekten Wert beim Mounten im Browser.
  ...
  return (
    <Text
      position={[0, 0, 12]}
      font="/assets/fonts/figtreeblack.ttf" // WICHTIG: Stellen Sie sicher, dass diese Datei existiert.
      fontSize={fontSize}
      letterSpacing={-0.05}
      outlineWidth={0}
      outlineBlur="20%"
